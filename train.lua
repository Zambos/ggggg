...
local MIDIUtils = require 'util.MIDIUtils'
...
cmd:text('Options')
-- data
cmd:option('-data_dir','data/midi','data directory. Should contain the file input.txt with input data')
-- model params
...
local loader = MIDIUtils.create(opt.data_dir, opt.batch_size, opt.seq_length, split_sizes)
...
if string.len(opt.init_from) > 0 then
    ...
    if not (checkpoint_vocab_size*6 == vocab_size) then
        vocab_compatible = false
