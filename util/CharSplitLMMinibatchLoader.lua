...
function CharSplitLMMinibatchLoader.create(data_dir, batch_size, seq_length, split_fractions)
    ...
    if (path.exists(vocab_file) or path.exists(tensor_file)) then
    ...
    --self.x_batches = self.split(self.x_batches, data, seq_length, 55, 1)
    --print(data:view(batch_size,-1))
    print(self.x_batches[1])
    self.nbatches = #self.x_batches
    self.y_batches = ydata:view(batch_size, -1):split(seq_length, 2)  -- #rows = #batches
    --self.y_batches = self.split(self.y_batches,ydata, seq_length, 55, 1)
...
function CharSplitLMMinibatchLoader.split_tensor_by_char(in_tensor, char)
   local splitTensor = torch.ByteTensor(in_tensor.length())

end

function CharSplitLMMinibatchLoader.split(result, tensor, splitSize,char , dim)
   --print(tensor[1])
...
--print(result[0])
 ...
 -- print(tensor)
   while start <= tensor:size(dim) do
      i=start
      while start<=tensor:size(dim) 
      and tensor[i]==char
       and i<=tensor:size(dim) do
        i=i+1
      end
      local size = math.min(i, tensor:size(dim) - start + 1)
