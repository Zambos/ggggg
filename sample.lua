...
local ivocab = {}
local map ={}
-- ivocab is dictionary
--io.write("----Start iVocab /n")
----------------------------
--
--
--
...
--local temp2 = ivocab[-1]
--ivocab[-1]=nil
print(ivocab)
print(#ivocab.."   "..(#ivocab/6).."  "..temp)
--os.ext()
print((temp)/6)
--io.write("/n----Stop iVocab")
...
then
    ...
    gprint('--------------------------')
    --print(#ivocab)
    prediction = torch.Tensor(1, temp):fill(1)/(temp)
...
local out={}
 local my_score = {
    960,  -- ticks per beat
    {    -- first track
       {'patch_change', 1, 1, 1}, -- vibraphone on channel 1
...
for i=1, opt.length do
    ...
    for i=1,state_size do table.insert(current_state, lst[i]) end
    prediction = lst[#lst] -- last element holds the log probabilities
    --print(prev_char[1].."  "..temp.."   "..prev_char[1]%temp.." ".. prev_char[1]/temp)
    --print(#ivocab)
    --print(ivocab[prev_char[1]])
    print(prev_char[1])
    --print(ivocab[prev_char[1]])
    --out = (out.ivocab[prev_char[1]])
    if ivocab[prev_char[1]]~=nil and ivocab[prev_char[1]]~=1 then
      my_score[2][tempMidiI]={"note",lastTime,(math.floor(ivocab[prev_char[1]]/127)+1)*240,1,ivocab[prev_char[1]]%127,100}
      lastTime=lastTime+ (math.floor(ivocab[prev_char[1]]/127)+1)*240
      tempMidiI = tempMidiI+1
      --print(tempMidiI)
    end
end
--print (my_score)
local midifile = assert(io.open('fak.mid','wb'))
--midifile:write(out)
